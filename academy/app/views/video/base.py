...
from academy.app.serializers.video import VideoPageDataSerializer
...
class VideoListCreateAPIEndpoint(BaseViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        logger.info("video_created", video_id=video.id,
...
class VideoDetailAPIEndpoint(BaseAPIView):
    ...
    def get(self, request, *args, pk):
        ...
        """
        logger.info("video_detail_requested",
                    requested_by=request.user.id, role=request.user.role)
        ...
        video = (self.get_queryset()
                 .select_related(
                    'course_content',
                    'course_content__course_offering',
                    'course_content__course_offering__teacher',
                    'course_content__course_offering__grade_level',
                    'course_content__course_offering__subject',
                )
            .get(pk=pk))

        logger.info("video_detail_loaded",
                    requested_by=request.user.id, role=request.user.role)
    ...
    def patch(self, request, pk):
        ...
        logger.info("video_updated", video_id=pk,
                    requested_by=request.user.id, role=request.user.role)
    ...
    def delete(self, request, pk):
        ...
        logger.info("video_deleted", video_id=pk,
                    requested_by=request.user.id, role=request.user.role)
